...
# Switche to ordering by groups of cells
...
library(writexl)
x <- lapply(res_IDEASv1_active, function(x) {as.data.frame(x %>% arrange(Coefficient_P_value) %>% select(EID, name, Coefficient, Coefficient_std_error, Coefficient_P_value, FDR))})
write_xlsx(x, format_headers=T, col_names = T, path = here("res/table_S4.xlsx"))
